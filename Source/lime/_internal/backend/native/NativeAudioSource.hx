...
class NativeAudioSource
{
	...
	public function play():Void
	{
		...
		/*var pos = getPosition();
			...
			trace('createFilter error ? ' + AL.getErrorString());

			AL.filteri(filter, AL.FILTER_TYPE, AL.FILTER_LOWPASS);
			trace('filteri error ? ' + AL.getErrorString());

			AL.filterf(filter, AL.LOWPASS_GAIN, 0.5);
			trace('filterf LOWPASS_GAIN error ?' + AL.getErrorString());
			
			AL.filterf(filter, AL.LOWPASS_GAINHF, 0.5);
			trace('filterf LOWPASS_GAINHF error ?' + AL.getErrorString());
			/*
